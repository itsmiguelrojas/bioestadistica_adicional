...
.pull-right[
$r$ representa el coeficiente de correlación, el cual indica la fuerza de asociación entre dos variables. Este se escribe de la forma. Cuando hablamos de una población, su parámetro se expresa como $\rho$.

La regresión lineal se escribe de la forma:

$Y=b_{0}+b_{1}X+\color{blue}e\\Y=\beta_{0}+\beta_{1}X+\color{blue}\epsilon$
...
$\color{blue}{\sum\limits_{i=1}^{n}\left(Y_{i}-\bar{Y}\right)^{2}}=\color{blue}{\sum\limits_{i=1}^{n}\left(\hat{Y}_{i}-\bar{Y}\right)^{2}}+\sum\limits_{i=1}^{n}\left(Y_{i}-\hat{Y}_{i}\right)^{2}\\\color{blue}{SS_{tot}}=\color{blue}{SS_{R}}+SS_{res}$
