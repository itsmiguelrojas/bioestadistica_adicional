...
6. Regresión lineal, prueba de hipótesis de correlación, prueba de hipótesis de la pendiente y prueba de hipótesis de la intersección.
...
**5** Se calcula el estadístico de prueba:
...
```{r distribucion_6, fig.height=3.5}
 abalone.sex %>%
  ggplot(aes(x = sex, y = whole_weight, fill = sex)) +
  geom_violin(alpha = 0.5) +
  stat_boxplot(geom = 'errorbar', width = 0.2, linewidth = 1) +
...
abalone.sex %>%
  select(sex, whole_weight) %>%
  arrange(whole_weight) %>%
  mutate(ranking = 1:nrow(.)) %>%
  select(-whole_weight) %>%
  group_by(sex) %>%
  mutate(row = row_number()) %>%
  pivot_wider(names_from = sex, values_from = ranking) %>%
  select(-row) %>%
  relocate(Infante, .after = Hembra) %>%
  as.data.frame()
```
...
```{r}
dunn.test(x = abalone.sex$whole_weight, g = abalone.sex$sex, altp = T)
```
...
pigmentacion %>%
  ...
  labs(
    x = 'Edad (años)',
    y = 'Pigmentación (%)',
    title = 'Estimación de la edad de leones a través de la coloración del hocico'
  ) +
  theme_minimal()
```
...
```{r}
pigmentacion$proporcion_pigmentacion
```
...
```{r regresion_2, fig.height=3.5, message=FALSE}
# Cargar datos ----
pigmentacion <- read_xlsx('datos/whitman2004.xlsx')
...
as.numeric(pigmentacion.lm$residuals)
```

.pull-left[
```{r}
shapiro.test(pigmentacion.lm$residuals)
```
]

.pull-right[
```{r distribucion_residuales, fig.height=4.5}
hist(pigmentacion.lm$residuals, main='Distribución de residuales')
```
]
...
```{r}
cor.test(pigmentacion$edad, pigmentacion$proporcion_pigmentacion)
```

Intervalo de confianza de la intersección y la pendiente:

```{r}
confint(pigmentacion.lm)
```
...
```{r}
edad.ranking <- c(1,2,rep(5,5),8,9,10,11,12,13,14.5,14.5,16,17,18,19,20,21,22.5,22.5,24,25,26.5,26.5,28.5,28.5,30,31,32)
...
```{r}
pigmentacion.ranking <- c(12.5,7,2,8,9,11,17,14,5.5,15,3.5,16,10,3.5,5.5,18,12.5,23,1,24,19,27,28,29,30,20.5,22,25,26,20.5,32,31)

pigmentacion.ranking
```

Su diferencia:

```{r}
edad.ranking - pigmentacion.ranking
```

Lo elevamos al cuadrado:

```{r}
(edad.ranking - pigmentacion.ranking)^2
```
...
.pull-left[
```{r regresion_3, fig.height=8}
salida.campo %>%
  filter(muerto == 'No') %>%
  ggplot(aes(x = area.basal, y = altura)) +
  geom_point() +
  labs(
    x = 'Área basal (m²/ha.)',
    y = 'Altura (m)',
    title = 'Altura de individuos vegetales en función de su área basal',
    subtitle = 'Cerro El Volcán (CV) y Jardín Botánico de Caracas (JB)'
  ) +
  theme_minimal()
```
...
```{r}
cor(salida.campo[salida.campo$muerto == 'No',]$area.basal, salida.campo[salida.campo$muerto == 'No',]$altura, method = 'pearson')
```

**Spearman**:

```{r}
cor(salida.campo[salida.campo$muerto == 'No',]$area.basal, salida.campo[salida.campo$muerto == 'No',]$altura, method = 'spearman')
```
...
.pull-left[
```{r regresion_4, fig.height=8}
salida.campo %>%
  filter(muerto == 'No') %>%
  ggplot(aes(x = log10(area.basal), y = log10(altura))) +
  geom_point() +
  labs(
    x = 'log(Área basal (m²/ha.))',
    y = 'log(Altura (m))',
    title = 'Altura de individuos vegetales en función de su área basal',
    subtitle = 'Cerro El Volcán (CV) y Jardín Botánico de Caracas (JB)'
  ) +
...
```{r}
cor(log10(salida.campo[salida.campo$muerto == 'No',]$area.basal), log10(salida.campo[salida.campo$muerto == 'No',]$altura), method = 'pearson')
```

**Spearman**:

```{r}
cor(log10(salida.campo[salida.campo$muerto == 'No',]$area.basal), log10(salida.campo[salida.campo$muerto == 'No',]$altura), method = 'spearman')
```
...
Propuesta por [Boos y Stefanski (2013)](https://doi.org/10.1007/978-1-4614-4818-1), esta regla ofrece un enfoque sobre el número de muestras que se deben tomar para estimar los p-valores. Para ello, se debe tomar un número de muestras que sea uno menos que un múltiplo de 100 (99, 999, 9999, etc.) con tal de mantener la integridad del nivel de significancia.

Más concretamente, los posibles p-valores a obtener son múltiplos de $1/\left(1+B\right)$.

Por ejemplo, con $B=99$, los p-valores del 1 al 10 son:

```{r}
1:10/(1+99)
```

Mientras que con $B=100$ son:

```{r}
round(1:10/(1+100), 4)
```

Si trabajamos con un valor de $\alpha=0.05$, bajo el segundo escenario es imposible un tener un p-valor igual a 0.05, lo que puede generar una ligera imprecisión al determinar la significancia estadística.
...
|:---|:---|
|No requiere suposiciones sobre la distribución de los datos.|Puede ser computacionalmente intensivo (aunque con las computadoras actuales, esto ya no es una limitante).|
|Se puede aplicar a cualquier estadística.|	Resultados pueden ser inestables con muestras pequeñas.|
|Proporciona intervalos de confianza más precisos en algunos casos.|Puede no funcionar bien con datos altamente sesgados.|

---
class: justify, animate__animated, animate__fadeIn

## Intervalo de confianza con bootstrap - Ejemplo

Como la aplicación del bootstrapping requiere muchas iteraciones, mostramos aquí un código de ejemplo con los datos de 50 muestras del peso total de abulones:

.code[
```{r echo=TRUE, fig.show='hide'}
# Número de iteraciones y vector para almacenar los promedios
B <- 9999
abalone.means <- vector('numeric', B)

# Semilla para reproducibilidad de los datos
set.seed(123)

# Bucle 'for' para realizar el remuestreo
for(i in 1:B){
  sample.1 <- sample(abalone.muestra, size = length(abalone.muestra), replace = T)
  
  abalone.means[i] <- mean(sample.1)
}

# Gráfico de la distribución del estadístico con su intervalo de confianza al 95%
abalone.means %>%
  as_tibble() %>%
  ggplot(aes(x = value)) +
  geom_histogram(bins = 14, fill = color.p[1], color = 'black') +
  geom_vline(xintercept = quantile(abalone.means, probs = c(0.025, 0.975)), linetype = 'dashed') +
  geom_vline(aes(xintercept = median(value)), linetype = 'dashed', linewidth = 2) +
  labs(
    x = 'Peso total (g)',
    y = 'Conteo',
    title = 'Distribución del promedio del peso total de abulones',
    subtitle = 'Remuestreo por bootstrap'
  ) +
  annotate(geom = 'label', x = 0.55, y = 1500, label = paste0('P2.5 = ', round(quantile(abalone.means, probs = c(0.025)), 4))) +
  annotate(geom = 'label', x = 0.965, y = 1500, label = paste0('P97.5 = ', round(quantile(abalone.means, probs = c(0.975)), 4))) +
  annotate(geom = 'label', x = 0.75, y = 2700, label = 'Mediana = 0.7502') +
  theme_minimal()
```
]
...
Lo cual genera la siguiente distribución:

```{r distribucion_boot_1, fig.height=3.5}
abalone.means %>%
  as_tibble() %>%
  ggplot(aes(x = value)) +
  geom_histogram(bins = 14, fill = color.p[1], color = 'black') +
...
## Prueba de hipótesis y cálculo del p-valor - Ejemplo

En este ejemplo, haremos el cálculo del p-valor para la diferencia de promedios entre las alturas de individuos vegetales de Cerro El Volcán (CV) y el Jardín Botánico de Caracas (JB):
...
```{r distribucion_boot_2, fig.height=3.5}
# Centrar en cero ----
alt.cv.ho <- alt.cv.sample - mean(alt.cv.sample)
alt.jb.ho <- alt.jb.sample - mean(alt.jb.sample)
