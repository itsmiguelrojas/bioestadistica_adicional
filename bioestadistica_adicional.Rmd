...
$T \leq T_{\text{crítico; }\alpha} \text{ (DC)}\\T \leq T_{\text{crítico; }2 \alpha} \text{ (CD)}\\-T \geq -T_{\text{crítico; }2 \alpha} \text{ (CI)}$
...
$T=min(R_{i}^{+},R_{i}^{-})=184.5$

$T_{\text{crítico; }0.05}=126$
...
$U \leq U_{\text{crítico};\:\alpha} \text{ (DC)}$
...
$U \leq U_{\text{crítico};\: 2 \alpha} \text{ (CD)}\\-U \geq -U_{\text{crítico};\: 2 \alpha} \text{ (CI)}$
...
Para este ejemplo usaremos los datos de [Fuller (2003)](https://doi.org/10.1242/jeb.00230) sobre la variabilidad de la temperatura arterial y cerebral de avestruces:
